        ...
        if (!autobot.testCaseNameOrFile.isEmpty()) {
            ...
            if (diagnostic.type != CommandLineParser::DiagnosticType::Undefined) {
                ...
                QMetaObject::invokeMethod(qApp, [this, task]() {
                        ...
                        //converter()->exportScoreMeta("Maria_Elena.mscz", "", "", false);
                        //converter()->fileConvert("el_choclo_c.mscz", "a.json", "", false,mu::String(""));
                        // converter()->fileConvert("el_choclo_c.mscz", "vid.mp3", "", false,mu::String("MuseScore Basic"));
                        //converter()->exportScoreVideo("el_choclo_c.mscz", "vid.mp4");
                    //     const char* command = "python add_audio.py";
                    //    // Execute the command
                    //     int status = std::system(command);
                    //     if (status != 0) {
                    //         std::cerr << "Error executing ffmpeg command" << std::endl;
                    //     }
                    //     std::cout<<"video ready"<<std::endl;
...
int App::processConverter(const CommandLineParser::ConverterTask& task)
{
    ...
    std::filesystem::path my_directory = "/home/andres/harmonicaFREE/public/mscore/";
    ...
    case CommandLineParser::ConvertType::File:
        ...
        // for (const auto& entry : std::filesystem::directory_iterator(my_directory)) {
        //     if (std::filesystem::is_regular_file(entry)) {
        //             // Print the filename
        //             //std::cout << entry.path() << std::endl;
        //             ret = converter()->fileConvert(QString::fromStdString(entry.path().string()), task.outputFile, stylePath, forceMode, soundProfile);
        //         }
        //     }
